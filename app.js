...
var playersState = new Array(null, null, null, null);
...
io.sockets.on('connection', function(socket)
{
	...
	socket.on('gameStarted', function(data)
	{
		...
		setInterval(function()
		{
			if (sendState == true)
			{
				io.sockets.emit('movePlayer', playersState);
				for (var i = 0; i < 4; i++)
					playersState[i] = null;
				sendState = false;
			}
		...
		setInterval(function()
		{
			if (gameStarted == true)
			{
				...
				if (aliveCount < 2)
				{
					io.sockets.emit('gameIsOver', winner);
					gameStarted = false;
					for (var i = 0; i < 4; i++)
						if (socketArray[i] != undefined)
	...
	socket.on('sendMoveKey', function(data)
	{
		if (data != null)
		{
			sendState = true;
	...
	socket.on('requestBombDrop', function(data)
	{
