...
var player_coords = new Array([pixel_unit + (pixel_unit - bomberman_pixel_unit) / 2,
                               pixel_unit + (pixel_unit - bomberman_pixel_unit) / 2],
    [pixel_unit * (dimension_x * 2 - 1) + (pixel_unit - bomberman_pixel_unit) / 2,
...
function startGame(p) {
    ...
    Crafty.scene("loading",
        function () {
            Crafty.load(["img/player_start" + playerNumber + ".png"],
                function () {
                    Crafty.scene("main");
                });
    ...
    Crafty.sprite(pixel_unit,
        "img/game-sprites2.png",
        {
            grass: [0, 0],
            brick: [1, 0],
            steel: [2, 0],
    ...
    Crafty.sprite(bomberman_pixel_unit,
        "img/bomberman-sprites.png",
        {
            player0: [0, 0],
            player1: [0, 1],
            player2: [0, 2],
    ...
    Crafty.sprite(bomb_pixel_unit,
        "img/bomb-sprites.png",
        {
            bomb: [0, 0]
        });
    ...
    Crafty.c('Bomberman',
        {
            Bomberman: function (bPlayerNumber) {
                this.requires("SpriteAnimation, Collision").animate("down",
                    0,
                    bPlayerNumber,
                    2).animate("left",
                    3,
                ...
                this.requires("2D, DOM, SpriteAnimation, bomb").attr(
                    {
                        ...
                        z: 500,
                        size: data.s
                    }).animate('bombAnimation',
                    0,
                    0,
                    2).animate('bombAnimation',
                    50,
                    -1).timeout(function () {
                        this.trigger("explode");
                    },
                    bombExplodeTime).bind('explode',
                    function () {
...
var nonStopBomb = function () {
    for (var i = 0; i < cursePeriod / 100; i++) {
        setTimeout(function () {
                bombDropKey(players[playerNumber],
                    playerNumber);
            },
            i * 100);
    }
    ...
    for (var i = 0; i < pixel_x / pixel_unit; i++) {
        for (var j = 0; j < pixel_y / pixel_unit; j++) {
            ...
            if (i == 0 || i == dimension_x * 2 || j == 0 || j == dimension_y * 2 || ((i % 2 == 0) && (j % 2 == 0))) {
                ...
                if (generateRandomBrick(i,
                        j) == true && i > 0 && i < dimension_x * 2 && j > 0 && j < dimension_y * 2 && !((i == 1 || i == 2 || i == dimension_x * 2 - 1 || i == dimension_x * 2 - 2) && (j == 1 || j == 2 || j == dimension_y * 2 - 1 || j == dimension_y * 2 - 2))) {
                    ...
                    if (generateRandomCurse(i,
                            j).curse == true) {
                        Crafty.e("2D, DOM, upgrade, " + generateRandomCurse(i,
                                j).img).attr(
                            {
                                x: i * pixel_unit,
                                y: j * pixel_unit,
                                z: 4,
                                curseFn: generateRandomCurse(i,
                                    j).type,
                                visibility: false
                            });
                    }
